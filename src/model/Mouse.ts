import { DTD_BASE_KEY, getClosestDtdNode, isValidNumber, removeGhostElStyle, setMoveElStyle, sortMouseEvents } from '../constant'
...
export class Mouse {
  ...
  insertNode(
    ...
    const insertNodes = sourceNodes.map(node => {
      node.parent = parent
      // assetsMap config
      const component = assetsMap[node.componentId]
      let droppable = false
      if (component?.configure?.slots?.length) {
        droppable = !!component.configure.slots.find(s => s.name === 'default')
      }
      // copy
      if (type === DragNodeType.COPY) {
        node = new DtdNode({ ...node, droppable, dragId: '' }, parent)
      }
    ...
    if (isContainer) {
      // if (this.droppable && !this.placeholder && !node.children?.length) {
      //   const placeholderNode = new DtdNode({ componentName: DefaultComponentName, props: {
      //     [DTD_BASE_KEY]: this.dragId
      //   }, children: [], droppable: true, placeholder: true }, this)
      //   this.children = [placeholderNode]
      // }
      if (parent.children.find(node => node.placeholder)) {
        parent.children = insertNodes
      } else {
        insertBefore
