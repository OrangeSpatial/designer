<template>
    <a-collapse v-model:activeKey="activeKey" ghost>
        <a-collapse-panel v-for="item in Object.keys(categories)" :key="item" :header="item">
            <DragToDrop :drag-type="DragNodeType.COPY" nodeClass="dtd-item" class="dtd-copy-nodes"
                :data="categories[item]">
                <template #default="{ item }">
                    <div class="copy-item">
                        <div class="dtd-copy-icon"></div>
                        <div class="dtd-node-name">{{ item?.componentName }}</div>
                    </div>
                </template>
            </DragToDrop>
        </a-collapse-panel>
...
import { AssetsKeyByIdSymbol } from "../../../constant";
import DragToDrop from "../../dragger/DragToDrop.vue";
import { DragNodeType } from "../../../model";
...
const assetsById = inject(AssetsKeyByIdSymbol, {})
